(function ($) {
    $.et_simple_slider = function (el, options) {
        var settings = $.extend({
            slide: '.et-slide',
            arrows: '.et-slider-arrows',
            prev_arrow: '.et-arrow-prev',
            next_arrow: '.et-arrow-next',
            controls: '.et-controllers a',
            control_active_class: 'et-active-control',
            ...
            manual_arrows: '',
            append_controls_to: '',
            controls_class: 'et-controllers',
            slideshow: false,
            slideshow_speed: 7000,
            show_progress_bar: true,
            tabs_animation: false
        }, options);
        var $et_slider = $(el), $et_slide = $et_slider.find(settings.slide), et_slides_number = $et_slide.length,
            et_fade_speed = settings.fade_speed, et_active_slide = 0, $et_slider_arrows, $et_slider_prev,
            $et_slider_next, $et_slider_controls, et_slider_timer, controls_html = '', $progress_bar = null,
            progress_timer_count = 0;
        $et_slider.et_animation_running = false;
        $.data(el, "et_simple_slider", $et_slider);
        $et_slide.eq(0).addClass('et-active-slide');
        if (settings.use_arrows && et_slides_number > 1) {
            if (settings.manual_arrows == '') $et_slider.append('<div class="et-slider-arrows"><a class="et-arrow-prev" href="#">' + settings.previous_text + '</a><a class="et-arrow-next" href="#">' + settings.next_text + '</a></div>'); else
        ...
        if (settings.use_controls && et_slides_number > 1) {
            ...
            if (settings.append_controls_to == '') $et_slider.append(controls_html); else
                $(settings.append_controls_to).append(controls_html);
            $et_slider_controls = $et_slider.find(settings.controls), $et_slider_controls.click(function () {
                if ($et_slider.et_animation_running) return false;
        ...
        function et_slider_auto_rotate() {
            if (settings.slideshow && et_slides_number > 1) {
                ...
                if ($et_slider.hasClass('et_slider_hovered') && $progress_bar.length && settings.slideshow && et_slides_number > 1) $progress_bar.stop();
        ...
        $et_slider.et_slider_move_to = function (direction) {
            var $active_slide = $et_slide.eq(et_active_slide), $next_slide;
            $et_slider.et_animation_running = true;
            if (direction == 'next' || direction == 'previous') {
                if (direction == 'next') et_active_slide = (et_active_slide + 1) < et_slides_number ? et_active_slide + 1 : 0; else
            ...
            if (!settings.tabs_animation) {
                ...
                $active_slide.addClass('et_slide_transition').css({
                    'display': 'block',
                    'opacity': 1
                }).delay(400).animate({opacity: 0}, et_fade_speed, function () {
    ...
    $(document).ready(function () {
        var $et_top_menu = $('ul.nav'), $comment_form = $('#commentform'),
            ...
            $tabs_widget = $('.widget_ettabbedwidget'), $tabs_widget_li = $tabs_widget.find('.categories-tabs li'),
            $recent_videos = $('.widget_etrecentvideoswidget'),
            $recent_videos_tabs = $recent_videos.find('.et-recent-videos-wrap li'), $et_container = $('.container'),
            et_container_width;
        ...
        $('#et-social-icons a').hover(function () {
            $(this).find('.et-social-normal').css({'opacity': 1}).stop(true, true).animate({
                'top': '-59px',
                'opacity': 0
            }, 300);
        ...
        (function et_search_bar() {
            var $searchform = $('.et-search-form'), $searchinput = $searchform.find(".search_input"),
        ...
        if ($categories_tabs_module.length) {
            ...
            $categories_tabs_module.find('.et-popular-mobile-arrow').click(function () {
                var $this_el = $(this),
                    ...
                    $slider_tabs = $slider.find('.categories-tabs li'), tabs_number = $slider_tabs.length,
        ...
        if ($recent_videos.length) {
            ...
            $recent_videos.find('.et-recent-video-scroll a').click(function () {
                var $this_el = $(this), direction = $this_el.hasClass('et-scroll-video-top') ? 'previous' : 'next',
        ...
        if ($home_popular_slider.length) {
            ...
            $home_popular_slider_tabs.click(function () {
                var $this_el = $(this), $home_tabs = $this_el.closest('.popular-posts-wrap').data('et_simple_slider'),
                    $tabs_container = $home_tabs.find('.popular-tabs ul'),
                    active_tab_index = $home_tabs.find('.popular-active').index(),
                    tabs_margin = parseInt($tabs_container.css('marginTop')), tabs_height = 0;
                if ($home_tabs.et_animation_running) return;
                if ($this_el.index() < 4) {
            ...
            $('.et-scroll-arrows a').click(function () {
                var $slider = $(this).closest('.popular-posts-wrap').data('et_simple_slider'),
                    $slider_tabs = $slider.find('.popular-tabs li'), tabs_number = $slider_tabs.length,
                    ...
                    direction = $(this).hasClass('et-scroll-arrows-bottom') ? 'next' : 'previous', next_tab;
                if ($slider.et_animation_running) return false;
                if (direction == 'next') {
                    next_tab = (current_tab + 1) < tabs_number ? current_tab + 1 : 0;
        ...
        function et_breadcrumbs_css() {
            ...
            $('.et_breadcrumbs_title').css('maxWidth', $('#breadcrumbs').outerWidth() - $('.et_breadcrumbs_content').outerWidth() - 5);
        }
        ...
        function et_popular_tabs_height_calculate() {
            ...
            $home_popular_slider.each(function () {
                var $this_el = $(this), $tabs = $this_el.find('.popular-tabs'),
                    $content = $this_el.find('.popular-posts'), $tabs_wrapper = $this_el.find('.et-popular-tabs-wrap');
        ...
        $comment_form.submit(function () {
            $comment_form.find('input:text, textarea').each(function (index, domEle) {
                ...
                if ($et_comment_label.length && $et_comment_label.is(':hidden')) {
                    if ($et_comment_label.text() == $et_current_input.val()) $et_current_input.val('');
                }
            });
        });
    });
